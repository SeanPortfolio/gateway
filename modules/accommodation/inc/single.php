            ...
            <div class="col-xs-12 hidden-sm hidden-md hidden-lg">
                <div class="room-info-sticker">
                    <div class="room-info-mid">
                        ...
                        <span class="quicklink-icons__stats room-info-mid-icon" $hidde><i></i>SQM <span class="data">$sqm</span></span>
                    </div>

                    <div class="price-sticker-bot">
                        <div class="price-quicklink btn" $hiddePrice><span class="from">FROM</span> <span class="pricetag">{$currencyIconView}</span><span class="num-p">$from_price</span><span class="currency_text">$currencyText</span></span></div>
                    </div>
                </div>
            </div>
        </div>
...
if(!empty($page_bookings->url) && ($resbook_id != null || $booking_engine_url != null))
{
   ...
   if(!empty($resbook_id))
   {
        $booknow  = '<a href="'.$page_bookings->url.'" title="" class="btn booking-btn'.$active_btn_cls.'">'.$ctaAccomBookingBtn.'</a>';
   }elseif(!empty($booking_engine_url))
   {
        $booknow   = '<a href="'.$booking_engine_url.'" target="_blank" title="" class="btn booking-btn'.$active_btn_cls.'">'.$ctaAccomBookingBtn.'</a>';
   }
}else{
    $booknow  = '';
}
